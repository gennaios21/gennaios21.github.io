...
function getMO() {
    ...
    if (readSingleFileCounter > 0) {
        ...
        lessons1 = parseInt(Number(document.querySelectorAll('input')[2].value));
        lessons2 = parseInt(Number(document.querySelectorAll('input')[3].value));
        mo = (document.querySelectorAll('input')[4].value);
    }
    ...
    lessons1 = (lessons1 >= 0 && lessons1 <= 39) ? lessons1 : window.alert("Αρ. μαθημάτων με συντελεστή 1.5 για πτυχίο: 27 μαθήματα κορμού + το πολύ 12 κατεύθυνσης")
    lessons2 = (lessons2 >= 0 && lessons2 <= 27) ? lessons2 : window.alert("Αρ. μαθημάτων με συντελεστή 2 για πτυχίο: 11 μαθήματα κορμού + τουλάχιστον 4 και το πολύ 16 κατεύθυνσης")
    ...
    let text = show.innerText
    if(school == 'ece')
        show.innerText = `Συγχαρητήρια μηχανευτή. Ο νέος Μ.Ο. σου είναι: ${new_mo}`
    else
        show.innerText = `Συγχαρητήρια! Ο νέος Μ.Ο. σου είναι: ${new_mo}`
